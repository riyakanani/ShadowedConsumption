...
public class SceneSequenceController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Initial states
        openBook.SetActive(false); // We will use girlBookFrames instead
        closedBook.SetActive(false);
        ...
        oldShadowFace.SetActive(true);
        newShadowFace.SetActive(false);
    ...
    void ChangeShadowFace()
    {
        if (oldShadowFace != null) oldShadowFace.SetActive(false);
        if (newShadowFace != null) newShadowFace.SetActive(true);
    }
    void TriggerArmMovement()
    {
        if (shadowArmAnimator != null) shadowArmAnimator.SetTrigger("GrabChips");
        StartCoroutine(StopSparklesAfterAnimation());
    }
    IEnumerator StopSparklesAfterAnimation()
    {
